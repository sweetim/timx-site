...
1. `app/page.tsx` renders a centered maker `Profile` component with profile copy, a responsive icon craft-card grid, social connect links, and selected work cards derived from the tool registry.
...
3. The selected-work cards link to static developer tool routes, clamp card descriptions to two lines, and a "View all tools" link goes to `/developer`.
...
8. Background Remover uploads or drops images into `UploadZone`; images are added to `sourceImages` and shown in the Source picker. Users can click "Remove Background" per image to start AI processing via `useBackgroundRemoverPool`, which creates one Web Worker per image for concurrent processing. Each worker calls `@imgly/background-removal` and reports progress independently. Users can also choose a static color plus tolerance and click "Remove Selected Color" to run a client-side Canvas API pass that makes matching pixels transparent. Users can browse between images while they process. Completed AI or static-color results are cached per source image ID so users can view previously processed results without reprocessing. The Source picker shows status badges (spinner for processing, checkmark for done, error icon for failures).
...
| `eslint.config.mjs` | ESLint flat config with generated output ignores |
...
| `app/developer/image-editor/_components/background-remover/index.tsx` | Background Remover client component with AI and static-color removal actions |
...
| `app/developer/image-editor/_components/background-remover/_components/static-color-removal-controls.tsx` | Color picker, tolerance slider, and action controls for static color transparency removal |
