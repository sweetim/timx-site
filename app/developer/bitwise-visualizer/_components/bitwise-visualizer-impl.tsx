...
const isBitWidth = (value: unknown): value is BitWidth =>
  typeof value === "number" && (WIDTHS as readonly number[]).includes(value)

const OPERATOR_IDS = OPERATORS.map((item) => item.id)
const isOperator = (value: unknown): value is Operator =>
  typeof value === "string" && (OPERATOR_IDS as readonly string[]).includes(value)

const STORAGE_PREFIX = "bitwise-visualizer:"
...
function usePersistentState<T>(
  ...
  const [state, setState] = useState<T>(() =>
    readStored(key, defaultValue, isValid),
  )
...
export default function BitwiseVisualizer() {
  const [width, setWidth] = usePersistentState<BitWidth>(
    "width",
    8,
    isBitWidth,
  )
  const [operator, setOperator] = usePersistentState<Operator>(
    "operator",
    "and",
    isOperator,
  )
