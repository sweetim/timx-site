...
function useImageCropper({ onResult, onSourceImage }: UseImageCropperOptions = {}) {
  ...
  const [aspectPreset, setAspectPreset] = useState<AspectRatioPreset>(DEFAULT_ASPECT_RATIO)
  ...
  useEffect(() => { displayDimensionsRef.current = displayDimensions }, [displayDimensions])
  ...
  const loadImage = useCallback(
    (file: File, notifySource = true) => {
      ...
      img.onload = () => {
        ...
        const containerHeight = Math.min(Math.max(wrapper.clientHeight, 400), MAX_DISPLAY_HEIGHT)
        const { displayWidth, displayHeight, scale } = getImageDisplayDimensions(
          img.naturalWidth, img.naturalHeight, containerWidth, containerHeight,
        )
        const ratio = ASPECT_RATIOS.find((r) => r.preset === DEFAULT_ASPECT_RATIO)?.ratio ?? null
  ...
  const handleAspectRatioChange = useCallback(
    (preset: AspectRatioPreset) => {
      ...
      const ratio = ASPECT_RATIOS.find((r) => r.preset === preset)?.ratio ?? null
      setCrop((prev) => {
        const withRatio = applyAspectRatio(prev, ratio)
        return clampCrop(withRatio, displayDimensions.width, displayDimensions.height, ratio)
      })
    },
    [displayDimensions],
  )
  ...
  const handleDimensionChange = useCallback(
    (dimension: "width" | "height", value: string) => {
      ...
      const ratio = ASPECT_RATIOS.find((r) => r.preset === aspectPreset)?.ratio ?? null
      ...
      const newCrop =
        anchorMode === "center"
          ? { x: cx - newDisplayW / 2, y: cy - newDisplayH / 2, width: newDisplayW, height: newDisplayH }
    ...
    (newDrag: DragState) => {
      ...
      const handleMove = (e: MouseEvent) => {
        ...
        const next = match(newDrag.type)
          .with("move", () => ({ x: sc.x + dx, y: sc.y + dy, width: sc.width, height: sc.height }))
          .otherwise(() =>
            match(anchorMode)
              .with("edge", () => computeEdgeResize(newDrag.type, dx, dy, sc, ratio))
              .with("center", () => computeCenterResize(newDrag.type, dx, dy, sc, ratio))
              .exhaustive(),
          )
      ...
      const handleUp = () => {
        ...
        if (anchorModeRef.current === "center" && newDrag.type !== "move") {
          ...
          setCrop((prev) => clampCropKeepCenter(prev, centerX, centerY, dims.width, dims.height, r))
        }
      }
  ...
  const resetLocal = useCallback(() => {
    match(status)
      .with({ phase: "idle" }, () => {})
      .with({ phase: "editing" }, ({ imageUrl }) => URL.revokeObjectURL(imageUrl))
